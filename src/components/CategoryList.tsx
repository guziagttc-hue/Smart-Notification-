    ...
    {categories.map(cat => (
      <div key={cat.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between h-[120px] hover:shadow-md transition cursor-pointer">
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">{cat.count}টি রিমাইন্ডার</p>
        </div>
      </div>
    ))}
    <div className="border-2 border-dashed border-gray-200 p-4 rounded-2xl flex flex-col justify-center items-center h-[120px] hover:border-gray-300 hover:bg-gray-50 transition cursor-pointer group">
      ...
      <span className="text-xs font-semibold text-gray-500 mt-2">নতুন যুক্ত করুন</span>
    </div>
  </div>
);
