...
export const NewEventModal = ({ onClose, onAdd, reminderToEdit, onUpdate }: NewEventModalProps) => {
      ...
      <div className="w-full max-w-[360px] bg-white rounded-3xl shadow-xl border border-gray-100 p-6 font-bengali">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-[#092A54] text-lg font-bold flex items-center gap-2">
            <PlusCircle className="text-xl" /> নতুন ইভেন্ট যুক্ত করুন
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-gray-700 text-sm font-semibold mb-1.5">ইভেন্টের নাম / টাইটেল</label>
            <input 
              type="text" 
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder="যেমন: ডাক্তারের সাথে অ্যাপয়েন্টমেন্ট" 
          ...
          <div>
            <label className="block text-gray-700 text-sm font-semibold mb-1.5">বিস্তারিত বিবরণ</label>
            <input 
              type="text" 
              value={subtitle}
              onChange={(e) => setSubtitle(e.target.value)}
              placeholder="যেমন: Dr. Karim, Dec 21" 
              className="w-full bg-[#F4F6FA] border border-gray-200 rounded-xl py-2.5 px-4 text-sm focus:outline-none focus:border-blue-400 transition text-gray-800"
            />
          </div>

          <div>
            <label className="block text-gray-700 text-sm font-semibold mb-1.5">তারিখ ও সময়</label>
          ...
          <div>
            <label className="block text-gray-700 text-sm font-semibold mb-2">ধরণ / ক্যাটাগরি</label>
            <div className="grid grid-cols-3 gap-2">
              <label className={`flex flex-col items-center justify-center p-2.5 border rounded-xl cursor-pointer transition ${category === 'meeting' ? 'bg-blue-50 border-blue-200' : 'border-gray-200'}`}>
                ...
                <span className="text-xs font-semibold text-gray-700">মিটিং</span>
              </label>
              ...
              <label className={`flex flex-col items-center justify-center p-2.5 border rounded-xl cursor-pointer transition ${category === 'task' ? 'bg-amber-50 border-amber-200' : 'border-gray-200'}`}>
                ...
                <span className="text-xs font-semibold text-gray-700">কাজ</span>
              </label>
              ...
              <label className={`flex flex-col items-center justify-center p-2.5 border rounded-xl cursor-pointer transition ${category === 'call' ? 'bg-blue-50 border-blue-200' : 'border-gray-200'}`}>
                ...
                <span className="text-xs font-semibold text-gray-700">কল</span>
              </label>
            </div>
          </div>

          <div className="flex gap-3 pt-4">
            <button type="button" onClick={onClose} className="flex-1 bg-red-50 text-red-500 rounded-xl py-3 text-sm font-semibold hover:bg-red-100 transition active:scale-95">বাতিল করুন</button>
            <button type="submit" className="flex-1 bg-[#092A54] text-white rounded-xl py-3 text-sm font-semibold hover:bg-[#0c386e] transition active:scale-95">
              {reminderToEdit ? 'সম্পাদনা করুন' : 'সংরক্ষণ করুন'}
            </button>
          </div>
        </form>
      </div>
